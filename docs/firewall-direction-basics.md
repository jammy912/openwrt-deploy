...
#### 同 zone 多介面互通:靠「區域內轉送」

一個 zone 裡放**多個介面**時,它們之間能不能互通,由「區域內轉送」(zone `forward`)決定:

```
wgts zone: wg6ts(192.168.51.x) + wg7ts(192.168.52.x)   ← 同 zone 兩個介面
  區域內轉送 = ACCEPT → wg6ts ↔ wg7ts 互通(51 網段能連 52 網段)
  區域內轉送 = REJECT → 兩條隧道互相隔離(各自能上網+連路由器,但看不到對方)
```

**「區域內轉送 ACCEPT」是同 zone 介面互通的必要條件,但不一定充分**——還要:
1. **路由通**:兩網段間要有路由(同 zone、同一台通常自動有)。
2. **WG allowed_ips**(隧道介面才需):wg6ts 對端的 `allowed_ips` 要含對方網段,
   否則封包進不了隧道(這是 WG 層,不是 firewall 層)。

**設計判斷**:兩條隧道是「不相關的獨立接入」→ REJECT 隔離(安全,A 組看不到 B 組);
是「同一網路延伸、要互通」→ ACCEPT。**預設 REJECT(隔離)對多數獨立隧道是對的。**

**速記**:同 zone 介面互通→開「區域內轉送」;跨 zone 互通→開「區域 ⇒ 轉送」的 src→dest。
