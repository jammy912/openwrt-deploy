...
### 2.2 為什麼不做 v6 DBR（而是「擋 v6 DNS 逼走 v4」）

DBR 只有 v4 set。client 若走 IPv6，就繞過 DBR、以真實 v6 IP 直出 → 同戶失效。
「正規」解法是把 DBR 也做一套 v6（v6 set/打標/`ip -6 rule`/AAAA refresh + wg 隧道載 v6）。
**本方案刻意不做 v6 DBR**，改用「擋 v6 DNS 逼所有 Netflix 功能流量走 v4 DBR」。理由：

**做 v6 DBR 的三個脆弱點（都是 v4 沒有的風險）**：
1. **wg 隧道要能載 v6，且遠端家用出口要能轉發 v6**——這是最大門檻,且**在對端、不在你控制的
   機器上**。遠端家用 ISP 的 v6 前綴常動態變動、NAT66/前綴委派複雜,配不好 → v6 封包打進
   隧道進黑洞,你 debug 不到。(v4 只要遠端能 NAT44,家用路由器天生會,幾乎不出錯。)
2. **v6 前綴會漂移**——家用 ISP 的 v6 前綴週期性變動 → 隧道 v6 位址、`ip -6 rule`、v6 set
   都可能失效。要新寫「偵測前綴變動→重建 v6 rule」邏輯,v4 完全不需要。
3. **半通不通會靜默洩漏**(最危險)——v6 打了標但 wg v6 路由沒建好 → 封包黑洞 or 回落本地
   v6 直出(洩真實 v6 IP)。變成「有時走對、有時洩漏」的**間歇性同戶失效**,比穩定壞更難抓。
   v4 DBR 有 `DBR_SkipMissingIface` 保護(介面不在就不打標),v6 多一條路徑後這保護要重寫。

**加上維護成本翻倍**:每改 DBR 邏輯,v4/v6 兩套都要改都要測;busybox 上 `ip -6`/v6 nft
語法/AAAA 解析各有坑;v6 除錯工具支援差(v6 conntrack 比 v4 難查)。

**「擋 v6 DNS 逼走 v4」為何反而穩**:`Block-LAN-IPv6-ToRouter`(擋 v6 DNS)+ AAAA 擋 →
client 走不了 v6 DNS、Netflix 沒 AAAA → 功能全走 v4 → 用**已經很穩的 v4 DBR**。等於把
「v6 這條路」整個關掉,上面三個風險全部消失,且確定性正確。**client 一般 v6 上網不擋**
(見 §4),所以 Netflix App 需要的 v6 正常。

**「v4 會不會消失、逼死我?」不會**:v4 位址 2011 就分配完了,但靠 NAT/CGNAT 續命,15 年
來仍是主力(全球 v6 採用率約 40-45%,過半流量還是 v4)。只要世上還有大量「只支援 v4」的
用戶,Netflix 等服務就**不敢關 v4**(關了=拋棄過半客戶)。v4 會被服務端保底支援幾十年,
遠超這套方案/路由器的生命週期。**等 v6 真成主流那天,工具鏈也成熟了,上面三個風險被解決
了,再做 v6 DBR 才順**——現在做是拿現在的爛工具解未來的問題。

> 一句話:不做 v6 DBR 不是湊合,是正確的長期選擇。多做 v6 DBR = 多三個風險換一個你不需要
> 的功能(你要的是「同戶=走家用 IP」,v4 DBR 已做到)。
