...
IGNORE_BELOW_DBM=-85    # [V7.6 新增] 過弱訊號不納入計算 (避免遠處幽靈裝置把功率拉爆)。傳 -999 = 關閉
DEBUG_PUSH=0            # [V7.6 新增] 1=本次推播監控裝置名+dBm (debug 用), 0=不推
...
[ -n "$5" ] && IGNORE_BELOW_DBM="$5"     # 第 22 個參數: 過弱訊號門檻 (低於此值不納入計算)
[ -n "$6" ] && DEBUG_PUSH="$6"           # 第 23 個參數: debug 推播監控裝置名+dBm
...
hm_lookup_mac() {
    ...
    echo "$HEARING_MAP_JSON" | awk -v target="$lc_mac" -v ignore_dbm="$IGNORE_BELOW_DBM" '
        BEGIN { in_mac=0; depth=0; current_key=""; local_sig=""; max_remote=""; found_local=0; found_remote=0 }
        {
            ...
            if (depth<=0) {
                ...
                # 本機訊號過弱 → 視為 WEAK,呼叫端會跳過此裝置 (不納入功率計算)
                if (local_sig+0 < ignore_dbm+0) { printf "WEAK %d 0\n", local_sig; exit }
    ...
    for mac in $in_macs; do
        ...
        case "$d" in
            ...
            NOLOCAL)
                ...
                ;;
            WEAK)
                log "[HearingMap] 跳過 $mac (本機=${lsig}dBm < IGNORE_BELOW_DBM=${IGNORE_BELOW_DBM},不納入功率計算)"
                ;;
            NOTFOUND|*)
                kept="$kept $mac"
...
get_weakest_signal() {
    ...
    if [ "$USE_HEARING_MAP" = "1" ] && [ -n "$HEARING_MAP_JSON" ]; then
        ...
        local weakest_hm
        weakest_hm=$(echo "$HEARING_MAP_JSON" | awk -v macs="$MONITORED_MACS" -v pfx="$hm_prefix" -v ignore_dbm="$IGNORE_BELOW_DBM" '
            BEGIN {
            ...
            }
            {
                ...
                if (match(line, /"signal"[[:space:]]*:[[:space:]]*-?[0-9]+/)) {
                    ...
                    if (index(cur_key, "#")==0 && index(cur_key, pfx)==1) {
                        # 過濾過弱訊號 (避免遠處幽靈裝置把功率拉爆)
                        if (sig < ignore_dbm+0) next
    ...
    local weakest_rssi=$(
        ...
        done | awk -v macs="$MONITORED_MACS" -v batman="$BATMAN_ENABLED" -v local_macs="$BATMAN_LOCAL_MACS" -v ignore_dbm="$IGNORE_BELOW_DBM" '
        BEGIN {
            # 將監控 MAC 字串拆解為陣列
    ...
    if [ $change_occured -eq 1 ]; then
        if [ "$FIRST_RUN" -eq 1 ]; then
            ...
            for _radio in $UCI_RADIO_2G $UCI_RADIO_5G; do
                ...
                _phydev=$(echo "$_radio" | sed 's/radio/phy/')
                for _dev in $(iwinfo | awk '/'"$_phydev"'-/{print $1}' | head -1); do
                    # ★ 必須對 phy 設定,不能對 dev(2026-08-29 x60pro/MT76 實測):
                    #     iw dev phy1-ap0 set txpower fixed 1000 → exit 0 但硬體不動(停在 4)
                    #     iw phy  phy1     set txpower fixed 1000 → 硬體確實變成 10 ✅
                    #   對 dev 設定驅動會默默忽略,導致整套 RSSI 動態調功率空轉卻無徵兆。
                    #   ⚠️ 不可改用 wifi reload 當替代方案:reload 會讓 hostapd 重新初始化,
                    #      usteer 隨之 "Disconnecting from local node" → hearing map 清空
                    #      → 下一輪讀不到 client 訊號 → 誤判無 client 降到最低功率 → 又要
                    #      調功率 → 每分鐘 reload 的惡性循環(實測 23:38-23:44 每分鐘一次)。
                    # ⚠️ iw 的 exit code 不可信,故設定後一律回讀 iw info 比對實際值。
                    if iw phy "$_phydev" set txpower fixed $((_pwr * 100)) 2>/dev/null; then
                        ...
                        if [ -z "$_actual" ]; then
                            ...
                            log "[警告] $_dev txpower 未達設定值：要求 ${_pwr} dBm,硬體實際 ${_actual} dBm(可能受法規/頻寬上限限制)"
                        fi
                    else
                        log "[警告] $_phydev txpower 設定失敗(iw 指令回傳非 0)"
                    fi
                done
            done
            uci commit wireless
