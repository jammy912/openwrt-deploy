...
# 基礎套件 (所有節點都裝)
REQUIRED_PKGS="coreutils-base64 openssl-util curl rsync zram-swap kmod-zram e2fsprogs fdisk luci-i18n-base-zh-tw luci-i18n-firewall-zh-tw luci-theme-material usteer luci-app-usteer luci-i18n-usteer-zh-tw iperf3"
...
if [ "$MESH_ROLE" = "gateway" ] || [ "$MESH_ROLE" = "hybrid" ]; then
    REQUIRED_PKGS="$REQUIRED_PKGS adguardhome dnsmasq-full luci-proto-wireguard luci-app-pbr luci-i18n-pbr-zh-tw luci-app-ddns luci-i18n-ddns-zh-tw qosify jq iputils-arping"
fi
