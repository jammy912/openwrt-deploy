    ...
    if grep -q '^config linecmd' "$TMP_DECRYPTED" 2>/dev/null; then
        ...
        while IFS= read -r _lc_line; do
            ...
            if /etc/myscript/linecmd-handler.sh "$_lc_action" "$@"; then
                ...
                push_notify "LineCMD ❌ ${_lc_action}${_lc_argdesc:+ ($_lc_argdesc)} (未知動作或失敗)"
            fi
        done < "$_LC_TSV"
        rm -f "$_LC_TSV"
    fi
